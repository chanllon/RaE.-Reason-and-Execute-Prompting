            ...
            if file_name.endswith('.json'):
                ...
                with open(file_path, 'r', encoding='utf-8') as file:
                    ...
                    if Trans:
                        ...
                        print(question_text)
                        word_count = len(question_text)
                    correctans = is_numeric(data["choices"][data["label"]])
            if file_name.endswith('.png'):
                image_path=os.path.join(root, file_name)
                if correctans:
                    ddata = {'id': id, 'question': question_text, 'imge_path': str(image_path),
...
# 读取指定文件夹下的数据
# folder_path = 'D:\Pythonfile\MGeo-MLLMs\Data\GeoQAA+'
