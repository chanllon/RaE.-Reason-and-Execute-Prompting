# -*- coding:utf-8 -*-
import PALMGeo
from PALMGeo.prompt import geometry_prompt
import re
import os
...
def GPT4PAL(image_path,question):
  ...
  prompt = geometry_prompt.Geometry_Prompt.format(question=question)
  answer = interface.run(Mode,image_path,prompt)
  # print(answer)
  return answer


def QwenplusPAL(image_path,question):
  prompt = geometry_prompt.Geometry_Prompt.format(question=question)
  interface = PALMGeo.interface.ProgramChatInterface(
    # model='gpt-3.5-turbo-instruct',
    model='qwen-vl-plus',
...
def GeminPAL(image_path,question):
  prompt = geometry_prompt.Geometry_Prompt.format(question=question)
  interface = PALMGeo.interface.ProgramChatInterface(
    # model='gpt-3.5-turbo-instruct',
